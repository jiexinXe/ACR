...
def main():
    ...
    # ==== DWMM: Disagreement-Weighted Max-Margin ====
    parser.add_argument('--dwmm', action='store_true', help='enable DWMM loss on unlabeled data')
    parser.add_argument('--dwmm-tau0', type=float, default=0.20, help='target margin tau0 for hinge (logit top1-top2)')
    parser.add_argument('--dwmm-alphaD', type=float, default=1.0, help='weight for disagreement (JS) in w(x)')
    parser.add_argument('--dwmm-betaM', type=float, default=1.0, help='weight for boundaryness (min top1-top2 prob-gap) in w(x)')
    parser.add_argument('--dwmm-lambda', type=float, default=0.5, help='loss weight for DWMM term')
    # 需要更平滑的话可换 softplus 版本
    parser.add_argument('--dwmm-softplus', action='store_true', help='use softplus surrogate instead of hinge for margin')
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            # ====== DWMM: Disagreement-Weighted Max-Margin (unlabeled weak) ======
            L_dwmm = torch.zeros([], device=args.device)

            if args.dwmm:
                # 两头在无标弱增强上的 logits
                z1 = logits_u_w          # [B, C]  标准头
                z2 = logits_u_w_b        # [B, C]  平衡/校正头

                # 概率（只用于权重与信号，避免泄梯度：detach）
                p1 = torch.softmax((z1.detach())/args.T, dim=1)
                p2 = torch.softmax((z2.detach())/args.T, dim=1)

                # 分歧：JS(p1, p2)
                mprob = 0.5 * (p1 + p2)
                js = 0.5 * (
                    (p1 * (p1.add(1e-12).log() - mprob.add(1e-12).log())).sum(dim=1) +
                    (p2 * (p2.add(1e-12).log() - mprob.add(1e-12).log())).sum(dim=1)
                )  # [B]

                # 边界性：两头的概率 top1-top2 gap（越小越靠边界）
                top2_1 = p1.topk(2, dim=1).values
                gap1 = top2_1[:, 0] - top2_1[:, 1]
                top2_2 = p2.topk(2, dim=1).values
                gap2 = top2_2[:, 0] - top2_2[:, 1]
                gap_min = torch.minimum(gap1, gap2)  # [B]

                # 标准化（批内稳健）
                js_n  = (js - js.mean())  / (js.std()  + 1e-6)
                gap_n = (gap_min - gap_min.mean()) / (gap_min.std() + 1e-6)

                # 样本权重 w(x) = sigmoid(alphaD * JS - betaM * gap) * gamma
                w = torch.sigmoid(args.dwmm_alphaD * js_n - args.dwmm_betaM * gap_n)

                # —— 质量门（能量 & 熵），避免放大明显噪声 ——  #
                with torch.no_grad():
                    # 以融合 logits 计算能量与熵
                    z_co = 0.5 * z1 + 0.5 * z2
                    energy = -torch.logsumexp(z_co, dim=1)  # 越低越自信
                    prob_co = torch.softmax(z_co, dim=1)
                    entropy = -(prob_co * (prob_co + 1e-12).log()).sum(1)

                # 经验阈值：能量 < e_th 且 熵 < h_th 才通过（也可只用其一）
                e_th = energy.median() - 0.5 * energy.std()
                h_th = entropy.median() - 0.5 * entropy.std()
                quality = (energy < e_th) & (entropy < h_th)  # [B]

                # —— 权重归一化（保持期望~1，稳住梯度占比） —— #
                w_raw = torch.sigmoid(args.dwmm_alphaD * js_n - args.dwmm_betaM * gap_n)
                w_raw = w_raw * quality.float()
                w = (w_raw / (w_raw.mean() + 1e-6)).detach()

                # 用融合 logits 计算 logit-margin；你也可以改成样本自适应融合
                z_co = 0.5 * z1 + 0.5 * z2
                top2_co = z_co.topk(2, dim=1)
                margin = top2_co.values[:, 0] - top2_co.values[:, 1]  # [B]

                # 目标间隔 tau（常数版；后续可做类自适应：tail类更大）
                tau = torch.full_like(margin, fill_value=args.dwmm_tau0)

                # hinge 或 softplus surrogate
                if args.dwmm_softplus:
                    mm = F.softplus(tau - margin)
                else:
                    mm = torch.clamp(tau - margin, min=0.0)
            ...
            # === quick diag for DWMM ===
            with torch.no_grad():
                frac_below_tau = (margin < tau).float().mean()
                w_mean = w.mean()
                w_pos = (w > 0.5).float().mean()
                js_mean = js.mean()
                gap_mean = gap_min.mean()
            # 可按需每 N 个 step 打印到进度条或 TensorBoard：
            if (batch_idx % 100 == 0) and (args.local_rank in [-1, 0]):
                print(
                    f"[DWMM] below_tau={frac_below_tau:.2f}, w_mean={w_mean:.2f}, w>0.5={w_pos:.2f}, js={js_mean:.3f}, gap={gap_mean:.3f}")

            loss = Lx + Lu + Lx_b + Lu_b
            if args.dwmm:
                loss = loss + args.dwmm_lambda * L_dwmm

            loss.backward()
            losses.update(loss.item())
            losses_x.update(Lx.item()+Lx_b.item())
            losses_u.update(Lu.item()+Lu_b.item())
            losses_fenqi.update(L_dwmm.item())
            optimizer.step()
            scheduler.step()
            if args.use_ema:
            ...
            if bar is not None:
                bar.suffix = (
                    ...
                    'Loss_u: {loss_u:.4f} | Loss_fq: {loss_fq:.4f}'
                ).format(
                    batch=batch_idx + 1,
                    size=args.eval_step,
                    bt=batch_time.avg,
